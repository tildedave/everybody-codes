...
const quest18 = @import("quest18.zig");
const quest19 = @import("quest19.zig");
const quest20 = @import("quest20.zig");
...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, quest, "quest15")) {
        if (std.mem.eql(u8, part, "1")) {
            std.debug.print("{d}\n", .{try quest15.answer1(allocator, lines)});
        }
        if (std.mem.eql(u8, part, "2")) {
            std.debug.print("{d}\n", .{try quest15.answer2(allocator, lines)});
        }
    }
    if (std.mem.eql(u8, quest, "quest16")) {
        if (std.mem.eql(u8, part, "1")) {
            std.debug.print("{s}\n", .{try quest16.answer1(lines)});
        }
        if (std.mem.eql(u8, part, "2")) {
            std.debug.print("{d}\n", .{try quest16.answer2(allocator, lines)});
        }
        if (std.mem.eql(u8, part, "3")) {
    ...
    if (std.mem.eql(u8, quest, "quest17")) {
        ...
        if (std.mem.eql(u8, part, "3")) {
            std.debug.print("{d}\n", .{try quest17.answer3(allocator, lines)});
        }
    }
    if (std.mem.eql(u8, quest, "quest18")) {
        if (std.mem.eql(u8, part, "1")) {
            std.debug.print("{d}\n", .{try quest18.answer1(allocator, lines)});
        }
        if (std.mem.eql(u8, part, "2")) {
            std.debug.print("{d}\n", .{try quest18.answer2(allocator, lines)});
        }
        if (std.mem.eql(u8, part, "3")) {
            std.debug.print("{d}\n", .{try quest18.answer3(allocator, lines)});
        }
    }
    if (std.mem.eql(u8, quest, "quest19")) {
        if (std.mem.eql(u8, part, "1")) {
            std.debug.print("{s}\n", .{try quest19.answer(allocator, lines, 1)});
        }
        if (std.mem.eql(u8, part, "2")) {
            std.debug.print("{s}\n", .{try quest19.answer(allocator, lines, 100)});
        }
        if (std.mem.eql(u8, part, "3")) {
            std.debug.print("{s}\n", .{try quest19.answer(allocator, lines, 1048576000)});
        }
    }
    if (std.mem.eql(u8, quest, "quest20")) {
        if (std.mem.eql(u8, part, "1")) {
            std.debug.print("{d}\n", .{try quest20.answer1(allocator, lines)});
        }
        if (std.mem.eql(u8, part, "2")) {
